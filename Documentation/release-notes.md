...
* After a flash jump, the search can be repeated with 'n'/'N' (for ':Flash') or ';'/',' (for ':Flash -f/-F/-t/-T'), like native '/' and 'f' searches.
* Flash jump labels are now drawn directly over the matched text (adornment layer), including multiple matches on one line, instead of in the glyph margin.
